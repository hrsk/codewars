...
function greet(language) {
    for (const languagesKey in languages) {
        if (language === undefined || language === '' || language === 'IP_ADDRESS_INVALID' || language === 'IP_ADDRESS_NOT_FOUND' || language === 'IP_ADDRESS_REQUIRED') {
            return languages['english']
        }
        if (languagesKey === language) {
            return languages[languagesKey]
        }
    }
}
